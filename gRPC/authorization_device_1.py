...
class ProfileUser(GrpcUser):
    ...
    def __init__(self, environment):
        ...
        self.users_stub = self.users_stub(self._channel)
        self.client = None
        self.user1 = None
        self.user2 = None
    ...
    def manage_user_registration(self):
        ...
        if self.user1 and not self.user2:
            self.delete_user(self.user1)
            self.user2 = self.register_new_user()
            self.obtain_device_authorization_code(self.user2)
            self.bind_device(self.user2, deviceSerial=self.device_serial)
    ...
    def delete_user(self, user):
        delete_request = users_pb2.DeleteUserProfileRequest()
        metadata = [('authorization', f"Bearer {user['access_token']}")]
        try:
            response = self.users_stub.DeleteUserProfile(delete_request, metadata=metadata)
            print(f"Successfully deleted user ПЕРВЫЙ!! {user['username']}")
            self.user1 = None  # Удаляем ссылку на первого пользователя
        except grpc.RpcError as e:
            print(f"Failed to delete user {user['username']}. Error: {e}")
