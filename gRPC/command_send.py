...
class CodeManagerUser(GrpcUser):
    ...
    def save_code_to_file(self, filename, action, code, one_time, expires, expired=False, success=True,
                          error_code=None):
        ...
        success_status = " - не удачно" if not success else " - удачно"
        error_status = f" с ошибкой {error_code}" if error_code else ""

        with open(filename, 'a', encoding='utf-8') as file:
            line = f'{action}; {code}; {one_time}; {expires} {expiration_status} {one_time_status}{success_status}{error_status}\n'
            file.write(line)
            print(f"Записана строка в файл: {line}")

            if error_code:
                print(f"ПОЛУЧИЛИ ОТВЕТ ОШИБКИ - {error_code}")
    @task
    ...
    def process_code(self, filename, code, one_time, expires, expired):
        ...
        success = response is None  # Успех, если response пустой или None
        ...
        self.save_code_to_file(filename, action, code, one_time, expires, expired=expired, success=success,
        ...
        if success:
            ...
            delete_success = delete_response is None  # Успех, если response пустой или None
            delete_error_code = None if delete_success else delete_response

            print(
                f"Записываем результат запроса на удаление в файл: код {code}, успех: {delete_success}, ошибка: {delete_error_code}")
            self.save_code_to_file(filename, delete_action, code, one_time, expires, expired=expired,
                                   success=delete_success, error_code=delete_error_code)
        else:
            print(f"Запрос на добавление кода {code} не удался. Код не будет удален.")
    ...
    def send_command(self, action, code, one_time, expires):
        ...
        try:
            ...
            print(response)  # Выводим полный ответ от сервера

            if not response or not response.ByteSize():
                print(f"{action}: Успех")
                return None
