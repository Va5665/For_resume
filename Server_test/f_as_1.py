...
class CagStatistics:
    ...
    async def fetch_all_content(self, session):
        ...
        while has_more_pages:
            ...
            async with session.post(url=self.all_content_url, headers=headers, json=params_film, params=data) as response_film:
                try:
                    ...
                    print("JSONDecodeError f_as_1.py : code")
                self.page += 1
        ...
        print(f"Время выполнения f_as_1.py : {elapsed_time} секунд")
    ...
    async def get_seria_by_id(self, session, episode_id):
        ...
        if not hasattr(self, 'logger f_as_1.py'):
            self.logger = logging.getLogger('my_logger f_as_1.py')
            self.logger.setLevel(logging.DEBUG)
            file_handler = logging.FileHandler('my_log.log f_as_1.py')
            file_handler.setLevel(logging.DEBUG)
            console_handler = logging.StreamHandler()
            console_handler.setLevel(logging.DEBUG)
            formatter = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s f_as_1.py')
        ...
        try:
            async with session.get(url, headers=headers, params=data) as response:
                if response.status == 200:
                    ...
                    self.logger.error(f'Response code f_as_1.py {response.status}: {await response.text()}')
                    return None
            ...
            error_message = f"ClientResponseError f_as_1.py : {e} - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
            self.logger.error(error_message)
        except aiohttp.ClientError as e:
            error_message = f"ClientError f_as_1.py : {e} - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
            self.logger.error(error_message)
        except JSONDecodeError as e:
            error_message = f"JSONDecodeError f_as_1.py : {e} - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
            self.logger.error(error_message)
    ...
    async def save_content_to_database_2(self, list_all_films):
        try:
            ...
            for item in all_status_data:
                ...
                print("provider_id f_as_1.py :", provider_id)
